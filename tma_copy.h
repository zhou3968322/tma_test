...
__global__ static void __launch_bounds__(kNumThreads, 1)
  ...
  cutlass::arch::fence_view_async_shared();
    ...
    // Print shared memory values after TMA load
  if (threadIdx.x == 0 && blockIdx.x == 0 && blockIdx.y == 0 && lane_predicate)
  {
    printf("blockDim.x: %d, blockDim.y: %d, gridDim.x: %d, gridDim.y: %d\n", blockDim.x, blockDim.y, gridDim.x, gridDim.y);
    constexpr int PRINT_ROWS = 4;
    constexpr int PRINT_COLS = 4;
    for (int m = 0; m < PRINT_ROWS; ++m)
    {
      for (int n = 0; n < PRINT_COLS; ++n)
      {
        // ???????SMEM tile?????????
        if (m < get<0>(tileShape) && n < get<1>(tileShape))
        {
          Element val = sS(m, n); // CUTE tensor???(?, ?)
          printf("SMEM[%2d, %2d] = %.2f\n", m, n, val);
        }
      }
    }
    printf("\n");
